...
把高危命令审批从站内点单升级为「审批魔方推飞书交互卡片 → 人在卡片上批 → 回调把工单置为通过/驳回」。
回调**不执行命令**(ADR 0010):通过后由有权者回到「审批」页点执行。站内审批仍作兜底,两条通道由原子认领
保证同一单只被决策一次。

> 与本手册无关的另一套飞书配置:「设置 › 通知」里的**飞书群通知**(`notify.lark*`),它只是往自定义机器人
> 推一张「有新审批单」的卡片(可带控制台回链、可签名),不接收回调、不影响审批结果。两套可同时开。
> 外发的审批单不止终端命令:执行窗口申请、含敏感字段的导出、发布单的审批阶段同样会推卡片。
...
- 验证一条:在这些环境对一条会触发审批的高危命令下发(如 `DROP TABLE ...`):
  1. 命令被拦截,生成审批单;
  2. 审批魔方在飞书推出交互卡片;
  3. 在卡片上点「通过」→ 审批魔方回调网关 → 审批单置 approved、发起人收到通知。
     **注意:命令不会在这一刻执行** —— 通过之后由发起人回到「审批」页自己执行
     (ADR 0010)。所以这一步的验证点是"通知到了、工单变成待执行",不是"表没了";
...
- **总开关关闭时回调端点同样拒绝**,不再靠残留的 `callbackSecret` 存活。
- 一旦网关本地记录了厂商的 `task_id`,回调**必须**带且匹配,否则 `40300`;`operator` 截断到 128 字节。
- 回调只会把工单置为 approved / rejected,不触发执行。
...
- **已实现**:网关侧超时自动驳回(`approval.onTimeout=auto-reject`)与发布单终止作废审批时,会向审批魔方
  `PATCH` 回写 cancel,飞书卡片随之失效(`service/external_approval.go` `cancelExternalApproval`)。
- **未实现**:执行结果 `/reply` 回帖到飞书原卡片——待厂商确认回传的 `om_` message_id 能否命中原卡片,
  见 `.scratch/lark-approval-integration/issues/04-*`。
