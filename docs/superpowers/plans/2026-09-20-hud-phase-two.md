...
注释不能用现在时描述尚未实现之物。现在它实现了。

注释里已经写好了交接说明，**照它说的做**：「以后纵向滚动的容器(如 .ib-side)要按同一条
理由补高光线时，加进这句话来，不要另起一条」。所以把

```
     目前只有 .c-table 一个,它是横向滚动(overflow-x: auto)。以后纵向滚动的容器
     (如 .ib-side)要按同一条理由补高光线时,加进这句话来,不要另起一条 ——
     理由是"会滚",不是"横滚"。
```
...
```
     目前两个:.c-table(横向滚动,overflow-x: auto)与 .ib-side(纵向滚动的粘性
     侧栏)。再有会滚的容器,按同一条理由加进这句话,不要另起一条 ——
     理由是"会滚",不是"横滚"。
```

这一步是本任务的一部分，不是顺手 —— 那条注释现在才算说全。改完之后它必须仍然
**句句为真**：`.ib-side` 此刻确实已经有 `background-image` 了（就是你这个任务加的），
而"目前两个"必须真的是两个。
...
终端页的三条测试都要先顶掉 WebSocket。文件顶部补一个 import：

```ts
import { installWsFake } from './wsFake'
```

并在本段落里用这个助手打开终端，而不是直接 `open(page, '/terminal')`：
