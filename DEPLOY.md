...
- **接受混跑**:先确认这一版的迁移对旧代码向后兼容(只加表/加列、不改名不删列、不加
  ...
  这段时间里新旧两版同时读写同一个库。
- **`--no-migrate`**:让 serve **不**应用迁移,由部署流程自己决定谁来跑。

  它不是"跳过检查"—— 启动时仍会验证 schema 已经是最新的,差一条就拒绝启动并点名
  差的是哪一条。这一点是有意的:如果它只是"什么都不做",那么"serve 不迁移、而人也
  忘了跑"就会把一台表不全的网关放上线,而那种进程会让 `/healthz` 变绿、让每个请求
  返 500 —— 正是把 serve 接上 Migrate 所要消灭的那个形状。

  典型用法:先 `./vela-gateway migrate` 一次,再逐台起 `./vela-gateway --no-migrate`。

**60 秒的锁等待上限**:迁移锁用的是 `pg_try_advisory_lock` 加一个最多 60 秒的重试。
同时滚动重启两台、而这一版的迁移本身跑超过 60 秒时,**第二台会启动即退出**,日志里是
`could not acquire migration lock (another migration is running)`。systemd / k8s 会把它
重启,最终能起来,但中间有一段 CrashLoop —— 而那句报错不会让人联想到"只是等超时了"。
迁移预计耗时较长时,用上面的停机窗口或 `--no-migrate`,不要靠滚动重启去撞这 60 秒。
