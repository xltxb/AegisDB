...
## 现场反馈修复(2026-07-31):按 backspace 刷屏出几十行重复内容

**现场**(`docs/Snipaste_2026-07-31_11-49-59.png`):在 PROD 终端里编辑一条很长的 `explain analyze SELECT …` 时按退格,屏幕被几十行几乎一模一样的内容填满,每行比上一行末尾少一个字符——**每按一次退格就多出一整行**。

### EF13【高】输入行超出终端宽度后,重绘变成"追加"而非"替换"(现场实证)
- 位置:`frontend/src/lib/lineEditor.ts` `redraw()` / `printAbove()` / `ctrlL()`
- 机制:`redraw()` 用 `\r\x1b[2K` 重绘 —— 回到行首 + **擦除当前这一行**。这个假设只在"输入行不换行"时成立。一旦 `提示符+缓冲区` 宽于终端就会折行,而光标停在**最后一行**:于是只擦掉最后一行、重新写出整条 `提示符+缓冲区`、再次折行,新内容落在旧内容**下方**。每次击键都留下一份残影,长按退格就刷满屏。
- 同一假设还存在于另外两处:`printAbove()`(带折行输入时打印带外输出,旧行留在上面 → 语句显示两遍)、`ctrlL()`(Ctrl+L 后重绘,且未复位行号,导致下一次重绘往上跑进已清空区域)。
- 修复:重绘改为**多行感知** —— 记住上次渲染时光标位于块内第几行(`renderedRow`),重绘时先 `\x1b[nA` 回到块首、`\x1b[0J` 擦到屏幕末尾,再写内容,最后按 `(提示符长度+光标位置)` 精确定位光标。`printAbove` 同样先擦整块;`ctrlL` 复位 `renderedRow` 后直接走 `redraw()`。
- **边界**:缓冲区宽度恰好是终端宽度整数倍时,终端处于 "pending wrap"(光标仍在满行末尾而非下一行首),行号算术会差一行 —— 补写一个空格提交折行,使两边一致。
- **测试手段**:在单测里写了一个**最小终端屏幕模型**(约 60 行:光标、折行、`\r`、CUU/CUD/CUF/CUB、EL、ED),对**渲染后的屏幕内容**断言,而不是断言发出了哪些转义序列 —— 这样测的是行为,重构不会误伤。模型第一次跑就复现了现场:10 次退格 → 屏幕上 10 份副本。新增 6 个用例(折行编辑只留一份、光标落点、行中插入、单行不受影响、printAbove、Ctrl+L)。
- **已知残留**:终端 resize 会改变 `cols`,而 `renderedRow` 是用旧 `cols` 算的,resize 后紧接着编辑可能错一次(下一次击键自愈)。未处理。
