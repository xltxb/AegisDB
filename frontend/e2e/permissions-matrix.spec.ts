...
import { ADMIN, ALL_MENUS, envelope, seedSession, stubShell } from './fixtures'
...
//
// In the Vue implementation it broke by losing its columns: the column template
// was moved onto a wrapper element that is a plain block, where
// `grid-template-columns` means nothing and no error is raised; the rows, which
// are the grids, were left with no template and collapsed to a single implicit
// column. Every cell then stacked vertically, the rows grew five times too tall,
// and two thirds of the table was blank.
//
...
const ROLE_DETAIL = {
  ...
  matrix: {
    ...
    release: { prod: 'approve', gli: 'approve', staging: 'allow', uat: 'allow', dev: 'allow' },
  },
  // `members` is a Member[] on the detail payload — the list shape above carries
  // a count under the same name, so it has to be replaced, not inherited.
  members: [],
  memberIds: [1],
  tags: [],
}
...
async function openPermissions(page: Page) {
  ...
  await expect(page.locator('.cap-head')).toBeVisible()
}

const headerCells = (page: Page) => page.locator('.cap-head .cap-th')
...
test.describe('capability matrix · role view', () => {
  test('there is one column per tier, and they are side by side', async ({ page }) => {
    ...
    await expect(headerCells(page)).toHaveText(TIERS.map((t) => t.displayName))
  ...
  test('one capability row shows its tiers beside each other, not stacked', async ({ page }) => {
    ...
    const row = page.locator('.cap-row').first()
    const cells = row.locator('.cap-cellwrap')
  ...
  test('a cell sits under its own tier column', async ({ page }) => {
    ...
    const cells = await page.locator('.cap-row').first().locator('.cap-cellwrap')
