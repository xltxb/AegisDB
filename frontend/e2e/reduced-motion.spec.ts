...
//
// The probe element is appended to a *live* document rather than asserted
// against a spinner the app happens to render, so the test does not depend on
// some screen being mid-load at the moment it looks.
...
test.describe('US#57 prefers-reduced-motion', () => {
  ...
  test('keeps the animation running when no preference is set', async ({ browser }) => {
    ...
    const dur = await spinDuration(page)
    await ctx.close()
    expect(seconds(dur)).toBeGreaterThan(0.3)
  })
})
