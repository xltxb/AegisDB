...
export function translateMetaSql(cmd: string, engine: string): MetaTranslation | null {
  ...
  /*
   * SQLite 单独一支,不能落进 MySQL 兜底。
   *
   * 兜底的意思是「认不出的引擎当 MySQL」,对 SQLite 会发出 `SHOW FULL TABLES`,
   * 而它连 SHOW 都不认 —— 敲 `\dt` 换回来一句 `near "SHOW": syntax error`。
   * 本地开发库与演示实例全是 SQLite,等于这套元命令在最常用的环境里不可用。
   */
  if (isSqlite) {
    ...
    switch (verb) {
      ...
      // \du / \dg 在 SQLite 上没有对应物 —— 返回 null,原样送去被拒绝,好过编一条
      // 查得出东西但答非所问的 SQL。
    }
    return null
  }
